...
| Z endstop | PG10 (^) | физический концевик, настроен |
...
└── snapshots/             ← точки восстановления
    ...
    └── snapshot-2026-03-10b/  ← XY хоминг работает (X=0 слева, Y=350 сзади) ✓
```
...
- [x] Хоминг X: G28 X → X=0 (левый концевик PG6) ✓
- [x] Хоминг Y: G28 Y → Y=350 (задний концевик PG9) ✓
- [x] homing_override: G28 Z сначала едет на X=9 Y=27 (под Z концевик)

### Нужно сделать
- [ ] Протестировать хоминг Z (концевик PG10 настроен, тест не проводился)
- [ ] quad_gantry_level — выровнять портал (требует Z хоминга)
- [ ] PROBE_CALIBRATE — Z offset (требует физического датчика на PB8)
...
## Координатная система (откалибровано 2026-03-10)

| Ось | Концевик | Пин | Позиция у концевика | dir_pin |
|-----|----------|-----|---------------------|---------|
| X | Левый | PG6 | X=0 | !PF12 |
| Y | Задний | PG9 | Y=350 | !PG1 (homing_positive_dir: true) |
| Z | Верхний | PG10 | Z=0 | !PG3 (homing_positive_dir: false) |

Позиция под Z концевик (для homing_override): **X=9, Y=27**
(9мм от левого края, 27мм от переднего края)
